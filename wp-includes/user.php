...
function send_confirmation_on_profile_email( $user_id = 0 ) {
	...
	/*
	 * DMPress: ported from WordPress 7.0.3 (email-change confirmation bypass).
	...
	// Reject when there is no authenticated user (ID 0), or the target is not the current user.
...
function wp_register_user_personal_data_exporter( $exporters ) {
	$exporters['wordpress-user'] = array(
		'exporter_friendly_name' => __( 'DMPress User' ),
...
function wp_user_personal_data_exporter( $email_address ) {
	...
	if ( isset( $user_meta['community-events-location'] ) ) {
		...
		$data_to_export[] = array(
			...
			'group_description' => __( 'User&#8217;s location data used for the Community Events in the DMPress Events and News dashboard widget.' ),
