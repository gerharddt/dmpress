...
final class WP_Theme implements ArrayAccess {
	...
	private static $default_themes = array(
		// DMPress: WordPress's bundled themes are not shipped. This list backs
		// WP_Theme::get_core_default_theme(), which resolves the fallback theme
		// when the active one is missing.
		'dmpstarter' => 'DMP Starter',
	);
	...
	public function __construct( $theme_dir, $theme_root, $_child = null ) {
		...
		if ( is_array( $cache ) ) {
			...
			/*
			 * DMPress: themes are metadata-only. When a theme has no style.css,
			 * accept a theme.json manifest ({ "name": …, "version": … }) as the
			 * theme definition. No templates are required — the front end is
			 * headless and never rendered by core.
			 */
			$theme_json_file = $this->theme_root . '/' . $this->stylesheet . '/theme.json';
			if ( file_exists( $this->theme_root . '/' . $this->stylesheet ) && is_readable( $theme_json_file ) ) {
				$this->headers                = $this->parse_theme_json_headers( $theme_json_file );
			...
			if ( ! file_exists( $this->theme_root . '/' . $this->stylesheet ) ) {
				...
				$this->errors = new WP_Error( 'theme_no_stylesheet', __( 'Stylesheet is missing. A theme requires either a style.css or a theme.json file.' ) );
			}
			$this->template               = $this->stylesheet;
			$this->block_theme            = false;
			$this->block_template_folders = $this->default_template_folders;
	...
	/**
	 * Builds a theme headers array from a DMPress theme.json manifest.
	 *
	 * DMPress themes are metadata-only. A theme.json manifest names the theme
	 * and its version; the remaining style.css-style headers are returned empty.
	 *
	 * @since DMPress 1.0.0
	 *
	 * @param string $file Absolute path to the theme's theme.json file.
	 * @return array Theme headers keyed like the style.css file headers.
	 */
	private function parse_theme_json_headers( $file ) {
		$headers         = array_fill_keys( array_keys( self::$file_headers ), '' );
		$headers['Name'] = $this->stylesheet;
		...
		if ( is_array( $data ) ) {
			...
			foreach ( $map as $json_key => $header_key ) {
				if ( ! isset( $data[ $json_key ] ) || '' === $data[ $json_key ] ) {
					continue;
				}
				$value                  = is_array( $data[ $json_key ] ) ? implode( ', ', $data[ $json_key ] ) : $data[ $json_key ];
