...
function create_initial_post_types() {
	...
	/*
	 * DMPress: the built-in 'post' post type is not registered by core. It ships
	 * instead as a default Content-Type Builder (Secure Custom Fields) entry —
	 * seeded once by dmpress_seed_default_content_types() — so administrators can
	 * edit or delete it like any other content type. Core is written to tolerate
	 * 'post' being absent.
	 */

	/*
	 * DMPress: the built-in 'page' post type is not registered by default.
	 * A "page" content type can be recreated on demand via the Content-Type
	 * Builder (Secure Custom Fields) if a site needs one. Core's page-related
	 * functions (is_page(), get_pages(), page-attributes support, etc.) are
	 * left intact so a recreated hierarchical 'page' type works as expected.
	 */
...
function use_block_editor_for_post_type( $post_type ) {
	// DMPress: the block editor has been removed; the classic editor is always used.
	return false;
}
