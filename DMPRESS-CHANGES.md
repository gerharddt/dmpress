...
> **Baseline:** stock WordPress 7.0. **Product version:** DMPress 1.0.0-beta.12 (pre-release).
> Internally `$wp_version` remains `7.0` for plugin/API compatibility; `$dmpress_version` (`1.0.0-beta.12`) is the product version shown to users.
...
- `$dmpress_version = '1.0.0-beta.12'` (product version shown in generator tags, admin footer, dashboard).
...
- **Logo replaced:** `assets/images/scf-logo.svg` drew the letters **S C F** as vector paths — invisible to a text search, but the most prominent SCF branding on screen. It is now `dmp-logo.svg`, a matching **DMP** monogram at the same weight, size and baseline; both references (the builder toolbar and the upgrade notice) were repointed.
