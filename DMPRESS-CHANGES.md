...
> **Baseline:** stock WordPress 7.0. **Product version:** DMPress 1.0.0-beta.81 (pre-release).
> Internally `$wp_version` remains `7.0` for plugin/API compatibility; `$dmpress_version` (`1.0.0-beta.81`) is the product version shown to users.
...
`category`, `post_tag` and `post_format` are no longer registered by `create_initial_taxonomies()`. Like `post`, they ship as **default Content-Type Builder entries** (see [§2](#2-added-by-dmpress-us)), so Categories and Tags can be renamed, deactivated, detached from Posts, or deleted outright.
...
**Post formats are gone too.** The `post_format` taxonomy (aside, gallery, quote, …) is a blog-*presentation* feature that does not fit a headless, structured-data CMS — it surfaced in the Content-Type Builder as a stray "Format" taxonomy on the Posts type, inherited from mirroring WordPress's `post` defaults rather than by choice. Its registration is removed and `post-formats` is dropped from the default Posts supports. The API (`get_post_format()` and friends) stays and no-ops safely, since it is guarded by `post_type_supports( 'post-formats' )`, which no type declares.
...
### "Available Tools" — removed from the Admin menu
The **Tools → Available Tools** submenu item is gone. Core's `tools.php` is an all-but-empty shell now — Press This and the category/tag converter are long removed, leaving only the `tool_box` hook — so it no longer earns a menu slot. `tools.php` still resolves by direct URL, and a plugin that renders into `tool_box` can register its own menu entry. The **Tools** group heading now anchors to `import.php` (its first remaining item) instead of `tools.php`, matching how the other group headings point at a real item.
...
The admin bar's **Comments node** (`wp_admin_bar_comments_menu`, the speech-bubble icon and pending-comment count) is no longer registered either — it linked to the removed global `edit-comments.php` screen. The function stays defined for plugin compatibility; only its `admin_bar_menu` registration in `class-wp-admin-bar.php` is dropped.
...
In its place, **`wp-includes/dmpress-update.php` adds a separate update channel keyed on `$dmpress_version`.** It reuses WordPress's entire update engine — `WP_Upgrader` / `Core_Upgrader` (unzip, copy, DB upgrade), the Dashboard → Updates screen, and core's `verify_file_signature()` — and only supplies what is DMPress-specific. Note that the **download itself is DMPress's own**: `dmpress_update_verify_package()` returns a file path from `upgrader_pre_download`, which short-circuits `WP_Upgrader::download_package()` so the package and its detached signature can be fetched and verified together. What DMPress supplies:
...
### "At a Glance" dashboard widget — rebuilt around content types
WordPress's stock At a Glance widget was removed (see §1) because it hard-coded post/page/comment counts for types DMPress no longer registers. A DMPress replacement (`wp-includes/dmpress-dashboard.php`) restores it in a form that fits the fork: it lists every **active content type** — whatever is registered through the Content-Type Builder, and nothing else — with its published count, each line linking to that type's filtered list screen (`edit.php?post_status=publish&post_type=…`) and showing that content type's own menu icon (rather than the stock dashboard's generic custom-type marker). The Content-Type Builder's own storage types (`acf-*`) and attachments are excluded, and a type the current user cannot edit is skipped. A deactivated content type is not registered at all, so it simply does not appear. The footer names the running **DMPress version** and links the active theme, and a "Search engines discouraged" line shows when indexing is turned off in Settings → Reading. It reuses the `dashboard_right_now` widget id, so the existing dashboard styling applies, and re-fires the `rightnow_end` / `activity_box_end` hooks so plugins that extended the widget keep working.
...
Observed as: a site updated to `1.0.0-beta.81` still showed the pre-restore Reading screen,
...
Fix: `get_core_checksums()` now returns `false` in DMPress (matching how the other
api.wordpress.org calls are neutralised). There are no wordpress.org checksums for a fork,
and the only correct baseline is the package itself — which is already Ed25519-verified
before it is unpacked. With no checksums, `update_core()` copies **every** file from the
package, so nothing is skipped. Both call sites (`update_core()` and
`Core_Upgrader::check_files()`) already handled a non-array return.

Note on roll-out: the update that *installs* this fix is executed by the **old** updater
already on disk, so that one transition still skips stock-identical files; the next update
(run by the fixed updater) copies everything and converges. Existing installs therefore
fully self-heal within one further release.
...
The updates screen's fallback message read *"You can update from WordPress 7.0 to WordPress 1.0.0-beta.81 manually"* — naming WordPress instead of DMPress, and reporting `$wp_version` (frozen at 7.0 for plugin compatibility) as the installed version. It now names DMPress and reports `$dmpress_version` on both sides. The adjacent "about to install … in English (US)" warning had the same product-name error.
...
### System email sender — `dmpress@`
Outgoing mail from `wp_mail()` defaulted to a `wordpress@<site>` From address (the display name was already "DMPress"). The local-part is now `dmpress@<site>` in both places pluggable.php sets it — the general sender and the comment-notification sender.
...
### Navigation menus — under Content Types
Navigation menus are restored, but grouped under **Admin → Content** rather than Appearance. A menu is structured content — the `nav_menu` taxonomy and its items — exposed over REST at `wp/v2/menus` and `wp/v2/menu-items` for a headless front end to fetch and render; it is data, not theming.

`nav-menus.php` hard-requires `current_theme_supports( 'menus' )`, which a metadata-only DMPress theme never declares, so the screen was previously unreachable. Menu support is now enabled in `wp-includes/dmpress-content-types.php` (`after_setup_theme`), and the **Menus** item moved from the Appearance group to the Content group in `wp-admin/menu.php`. There are no theme menu *locations* on a headless install — menus are addressed by id or slug through the API. Verified end to end: a menu created in the admin is returned by both REST endpoints.
...
- `$dmpress_version = '1.0.0-beta.81'` (product version shown in generator tags, admin footer, dashboard).
...
- **Asset cache-busting:** SCF's version never moves while the fork patches its built CSS/JS in place, so browsers kept serving stale copies of DMPress's changes. `includes/assets.php` folds `$dmpress_version` into the registered version string (`?ver=6.9.1-dmp1.0.0-beta.81`), so every release bumps the URL.
...
- **Default install content** (`wp-admin/includes/upgrade.php`) — the sample "Hello world!" post now reads "Welcome to DMPress…", and the accompanying sample comment is authored by "A DMPress Commenter" with a neutral e-mail and no author URL (was "A WordPress Commenter" linking to wordpress.org). The dormant sample-page copy was rebranded too. All of this default content was additionally **stripped of Gutenberg block markup** (`<!-- wp:paragraph -->` etc.), which would otherwise appear as literal comments in the classic editor now that the block editor is gone. The sample comment's body no longer says to *"visit the Comments screen in the dashboard"* — there is no global Comments screen in DMPress; it now points to the **Comments screen listed under its content type**, matching the per-content-type comment menus.
...
Security and bug fixes from the point releases after the fork (**7.0.1**, **7.0.2**, **7.0.3**) that land in code DMPress still ships:

- **`kses.php`** — `safecss_filter_attr()` decodes `style` attributes before filtering / re-encodes after (XSS hardening). *Security.*
- **`class-wp-query.php` + `class-wp-comment-query.php`** — `author__not_in` (and the comment query's `author__in` / `post_author__in` / `post_author__not_in`) sanitised via `wp_parse_id_list()` rather than the old `is_array()`-gated `array_map( 'absint', … )`, which let a **string** value skip sanitisation. This is the unauthenticated SQL-injection reachable over REST as `author_exclude` on `/wp/v2/posts` and `/wp/v2/comments`. **CVE-2026-60137** (per Patchstack advisory, fixed upstream in 7.0.2). *Security.* Verified present here and byte-identical to patched upstream.
- **`rest-api/class-wp-rest-server.php` + `rest-api.php`** — REST batch fixes: (a) the nested-dispatch guard (`serve_request()` refuses to re-enter while a dispatch is in flight); and (b) `serve_batch_request_v1()` keeps its `$requests` / `$matches` / `$validation` arrays index-aligned so a batched request's route, validation, permission callback and final callback all bind to the same handler — the index-desync being **CVE-2026-63030** (per Patchstack advisory, fixed upstream in 7.0.2), also unauthenticated over REST. *Security.* Verified byte-identical to patched upstream.
...
### WordPress 7.0.3 (August 2026)
The 7.0.3 security release fixed roughly a dozen issues; their relevance here varies because the fork removed the block editor and the server-rendered front end.

**Ported (code DMPress ships):**
...
- **Pre-auth reflected XSS on the login screen (CVE-2026-64638 / GHSA-52p2-r8wf-jcrf).** The obvious reflected sinks in `wp-login.php` — the interim-login `redirect_to`/`action` inputs and the page title — are **byte-identical between DMPress 7.0 and patched upstream master**, so the fix is in a non-obvious location and is deliberately not guessed at. Highest priority to finish.
...
### Update nag rebranded — "DMPress X is available"
The admin update-nag banner (`update_nag()` in `wp-admin/includes/update.php`), shown on every admin screen except the Updates page, read *"WordPress &lt;version&gt; is available! Please update now"* and linked the version to a `wordpress.org` documentation page that does not exist for the fork. It now reads **DMPress** and links to the release's own notes URL from the update manifest (`dmpress_notes_url`), falling back to the old link only if the manifest carries no notes URL. The version number itself was always correct (it comes from the update transient); only the product name and the release link were wrong.

### DMPress-originated fix — update signature check was bypassable (critical)
Found in a security review of DMPress's own code; **not** an upstream issue.

`dmpress_update_verify_package()` tested the package URL's **scheme before** identifying the package, and returned control to WordPress for anything that was not `http(s)://`:

```php
if ( false !== $reply || ! preg_match( '#^https?://#i', (string) $package ) ) {
	return $reply;   // <- unverified passthrough
}
```

`WP_Upgrader::download_package()` accepts `ftp://` **and** bare local paths, and `Core_Upgrader` calls it with `$check_signatures` hard-coded to `false`. So a package advertised as `ftp://…` (or as a local path that exists) skipped DMPress's Ed25519 gate entirely and was downloaded, unpacked and installed **with no signature verification at all** — full remote code execution on every install that applied it.

The manifest is fetched over HTTPS but is **itself unsigned**; it is deliberately not trusted to be honest about where the package lives. The signature is the root of trust, so it has to be enforced for our package no matter how the URL is spelled — which is precisely what the old ordering failed to do.

Fixed by identifying the package **first** and, once it is ours, guaranteeing every path ends in either a verified file or a `WP_Error`: the package and its detached signature must both be **HTTPS**, and the Ed25519 signature must verify against the pinned public key. Verified: `ftp://`, plaintext `http://`, a local path, and an HTTPS package with an `http://` signature are each now rejected with an explicit error, while an unrelated (non-DMPress) package still passes through untouched and an already short-circuited download is still respected.

### DMPress-originated hardening — front-page REST route (information disclosure)
`dmpress_front_entry()` feeds the **unauthenticated** `dmpress/v1/front-page` route and checked only for `publish` status. A published entry belonging to a post type that is not publicly viewable, or one that is password-protected, would still have had its title, slug and permalink returned to anonymous callers. Both are now treated as "no entry", exactly like an unset option.

### DMPress-originated hardening — Customizer bootstrap (defense-in-depth)
The two REST vulnerabilities above (**CVE-2026-60137** + **CVE-2026-63030**) are chained by the public *wp2shell* exploit into unauthenticated remote code execution: the SQL injection and batch permission-bypass are used to forge a `customize_changeset` row carrying an administrator's `user_id`, then a request bearing that changeset UUID makes `WP_Customize_Manager` process it **in the admin's context**, ending in a forged admin account and a self-deleting plugin that runs shell commands.

DMPress already blocks that chain at both foundational bugs (they are fixed, verified byte-identical to patched upstream). This adds a **second line of defense** at the bootstrap step itself, since the escalation machinery — the changeset bootstrap, oEmbed handling, `POST /wp/v2/users` — is otherwise still present. `_wp_customize_include()` (`wp-includes/theme.php`) now requires the **`customize` capability** before honouring the front-end/AJAX bootstrap triggers (`wp_customize=on`, `customize_changeset_uuid`), which upstream fires on *any* WordPress-booting request with no authentication. The admin Customizer is unaffected: `customize.php` loads via its own admin-page path (and already gates on that same capability), and the authenticated `customize_save` AJAX request still passes. Verified: unauthenticated requests carrying either trigger no longer bootstrap the manager, while an authenticated administrator still does. `pluggable.php` loads before the `plugins_loaded` hook that fires this, so the capability check is available at that point.
