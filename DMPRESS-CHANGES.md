...
> **Baseline:** stock WordPress 7.0. **Product version:** DMPress 1.0.0-beta.14 (pre-release).
> Internally `$wp_version` remains `7.0` for plugin/API compatibility; `$dmpress_version` (`1.0.0-beta.14`) is the product version shown to users.
...
- `$dmpress_version = '1.0.0-beta.14'` (product version shown in generator tags, admin footer, dashboard).
