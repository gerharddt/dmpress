...
> **Baseline:** stock WordPress 7.0. **Product version:** DMPress 1.0.0-beta.37 (pre-release).
> Internally `$wp_version` remains `7.0` for plugin/API compatibility; `$dmpress_version` (`1.0.0-beta.37`) is the product version shown to users.
...
**The Optional Category base / Tag base fields are shown per taxonomy.** They set the URL base for term archives, which reaches the front end through each term's REST `link` field — so they belong to the same URL contract. But `category` and `post_tag` are Content-Type Builder entries that can be deactivated or deleted, and the fields were rendering regardless: a site with Categories switched off still offered a Category base for a taxonomy whose REST route 404s. Each field now appears only while its taxonomy is registered, and the whole **Optional** section is skipped when neither is. Every other taxonomy sets its own rewrite slug in the Content-Type Builder; these two fields exist only for the defaults.

There is no data-loss risk here, unlike the settings allow-lists: `options-permalink.php` guards both save handlers with `isset()`, so a field that is not rendered is simply not submitted and its stored option is left alone. Verified by seeding a `category_base` with Categories deactivated and submitting the form — the value survived while `tag_base` took its posted change.
...
- `$dmpress_version = '1.0.0-beta.37'` (product version shown in generator tags, admin footer, dashboard).
...
- **Asset cache-busting:** SCF's version never moves while the fork patches its built CSS/JS in place, so browsers kept serving stale copies of DMPress's changes. `includes/assets.php` folds `$dmpress_version` into the registered version string (`?ver=6.9.1-dmp1.0.0-beta.37`), so every release bumps the URL.
