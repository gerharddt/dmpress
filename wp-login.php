...
function login_header( $title = null, $message = '', $wp_error = null ) {
	...
	$login_title = sprintf( __( '%1$s &lsaquo; %2$s &#8212; DMPress' ), $title, $login_title );
	...
	$login_header_text = empty( $login_header_title ) ? __( 'Powered by DMPress' ) : $login_header_title;
	...
	default:
			...
			if ( headers_sent() ) {
				...
				$user = new WP_Error(
					...
					sprintf(
						...
						__( '<strong>Error:</strong> Cookies are blocked or not supported by your browser. You must <a href="%s">enable cookies</a> to use DMPress.' ),
		...
		if ( $interim_login ) {
			...
			if ( isset( $_GET['loggedout'] ) && $_GET['loggedout'] ) {
				...
				$errors->add( 'updated', __( '<strong>You have successfully updated DMPress!</strong> Please log back in to see what&#8217;s new.' ), 'message' );
