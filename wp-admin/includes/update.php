...
function get_core_checksums( $version, $locale ) {
	...
	if ( $ssl && is_wp_error( $response ) ) {
		wp_trigger_error(
			...
			) . ' ' . __( '(DMPress could not establish a secure connection to WordPress.org. Please contact your server administrator.)' ),
...
function core_update_footer( $msg = '' ) {
	if ( ! current_user_can( 'update_core' ) ) {
		...
		return sprintf( __( 'DMPress %s' ), $GLOBALS['dmpress_version'] );
	}
...
function update_right_now_message() {
	...
	$content = __( 'DMPress %1$s running %2$s theme.' );
	...
	$msg .= sprintf( '<span id="wp-version">' . $content . '</span>', $GLOBALS['dmpress_version'], $theme_name );
