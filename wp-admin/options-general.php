...
if ( ! is_multisite() ) {
	$options_help .= '<p>' . __( 'Two terms you will want to know are the DMPress URL and the site URL. The DMPress URL is where the core DMPress installation files are, and the site URL is the address a visitor uses in the browser to go to your site.' ) . '</p>' .
		'<p>' . sprintf(
			/* translators: %s: Documentation URL. */
			__( 'Though the terms refer to two different concepts, in practice, they can be the same address or different. For example, you can have the core DMPress installation files in the root directory (<code>https://example.com</code>), in which case the two URLs would be the same. Or the <a href="%s">DMPress files can be in a subdirectory</a> (<code>https://example.com/wordpress</code>). In that case, the DMPress URL and the site URL would be different.' ),
			__( 'https://developer.wordpress.org/advanced-administration/server/wordpress-in-directory/' )
		) . '</p>' .
		'<p>' . sprintf(
			/* translators: 1: http://, 2: https:// */
			__( 'Both DMPress URL and site URL can start with either %1$s or %2$s. A URL starting with %2$s requires an SSL certificate, so be sure that you have one before changing to %2$s. With %2$s, a padlock will appear next to the address in the browser address bar. Both %2$s and the padlock signal that your site meets some basic security requirements, which can build trust with your visitors and with search engines.' ),
			'<code>http://</code>',
			'<code>https://</code>'
		) . '</p>' .
...
$options_help .= '<p>' . __( 'You can set the language, and DMPress will automatically download and install the translation files (available if your filesystem is writable).' ) . '</p>' .
...
if ( ! is_multisite() ) {
	...
	$sample_tagline = __( 'Just another DMPress site' );
} else {
...
<?php
/*
 * DMPress: no Site Icon row. The icon is a front-end favicon / app icon and
 * the front end is headless, so core never renders it. The 'site_icon' option
 * and get_site_icon_url() are untouched, so a headless app can still read it
 * and plugins can still set it.
 */
...
<tr>
<th scope="row"><label for="siteurl"><?php _e( 'DMPress Address (URL)' ); ?></label></th>
...
<p class="description" id="home-description">
		...
		printf(
			/* translators: %s: Documentation URL. */
			__( 'Enter the same address here unless you <a href="%s">want your site home page to be different from your DMPress installation directory</a>.' ),
			__( 'https://developer.wordpress.org/advanced-administration/server/wordpress-in-directory/' )
		);
		?>
</p>
